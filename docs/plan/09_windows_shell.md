...
<a id="vertical-scrollbars"></a>
### 垂直滚动条

- Source、Preview 各提供一条垂直滚动条；Split 两侧各一条，沿用 `06` 的语义滚动同步开关。
- 各内容面板右侧保留固定窄槽，正文布局、绘制与命中不进入窄槽；窄槽不随 Content Zoom
  缩放，不侵占原生窗口 resize 边界或 Split 同步按钮。短文隐藏滑块但保留槽宽，避免出现/消失导致正文反复换行。
- 内容超出一屏时常驻细滑块，鼠标进入窄槽或拖动时加粗；滑块有最小可抓取高度。
  拖动实时定位，拖到两端分别到文首/文末；点击轨道空白向对应方向翻一屏。
- 滚动仅改变视口，不改变 canonical text、generation、dirty、caret、selection 或 IME preedit。
  松开鼠标、失焦、切换视图或视口尺寸变化时结束拖动；鼠标移出窗口仍可完成当前拖动。
- Source 以逻辑行及行内折行偏移映射滑块，末端仅排版足够填满最后一屏的行并缓存边界；
  不为滚动条在每个 pointer event 遍历或排版全文。Preview 复用既有 document height。
- 未完成或过期的 Preview frame 不提供可操作滑块；旧滚动请求的后台回执不得使新位置回跳。
- 验证覆盖长文与单行折行、首尾、短文、缩放/resize、分栏同步开关、拖动退出与选区不变。
  DPI/主题下的实际可抓取性与不遮字属于桌面视觉验收。
