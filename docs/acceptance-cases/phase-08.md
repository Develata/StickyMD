...
| P08-A16 | Phase 8 copied-Release lifecycle | Automated | [`phase-08.ps1 -Runtime`](../../tools/smoke/phase-08.ps1) uses physical pointer drag plus real shell focus loss for left/top/right snap-auto-collapse-sensor-reveal, runs Right while Pin is ON, drives both top-corner priority cases, WM_CLOSE and same-directory wake; exact 700-ms guards and `Top > Left > Right` ties also have deterministic reducer coverage | AUTOMATED PASS |
