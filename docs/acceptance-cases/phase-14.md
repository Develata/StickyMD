...
| P14-A15 | USER 授权的 candidate defect correction 保持边界内、无新 runtime dependency 且有命名回归 | Automated | git path/dependency audit + source selection/source projection/preview relayout/native-drag-completion/direct-redock、signed `WM_NCLBUTTONDOWN` packing 与线性 semantic-conversion regressions | AUTOMATED PASS |
