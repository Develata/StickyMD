...
| `04_runtime_state_model.md` | 内部 authority、全局内容缩放配置权威 | AC-009、AC-013、AC-032 | `stickymd-win/{config/runtime.rs,config/coordinator.rs,flow/preferences.rs,app/{input,preview_runtime,presentation,window_runtime}.rs}`；`stickymd-render/{source/projection.rs,preview/pipeline.rs}` | constrained Config authority、source/preview relayout、no-generation/no-reparse tests PASS；real zoom/IME visual NOT TESTED |
...
| `07_editor_and_ime.md` | 源码输入、中文输入法、Undo/Redo、传统剪贴板快捷键、内容缩放输入 | AC-002、AC-003、AC-004、AC-009、AC-022、AC-031、AC-032 | `stickymd-render/src/source/*`; `stickymd-win/{instruction/intent.rs,flow/editor.rs,flow/preferences.rs,interaction,app/{input,preview_input,presentation}.rs}` | typed clipboard aliases、wheel accumulator、zoom authority/projection tests PASS；AC-003/004 real IME and real keyboard/clipboard remain NOT TESTED |
...
| `09_windows_shell.md` | dock、托盘、置顶、透明度、主题、多显示器、紧凑窗口、Tool Window | AC-019..029、AC-033..036 | `stickymd-win/{flow/window,config,app/{lifecycle,window_runtime,window_interaction,window_geometry_runtime,controls,toolbar_paint}.rs,platform/windows/{tool_window,tray,monitor,native_message,window_opacity,window_topmost}.rs}` | 220×120 geometry、24-DIP nearest/tie reducer、40% alpha and copied HWND Tool/App/NOACTIVATE facts PASS；real shell/IME/visual rows NOT TESTED |
| `10_performance_reliability.md` | 质量属性 | AC-022、AC-032、空闲/内存观察 | core/source/preview/math/image/export/window/zoom Release baselines；`stickymd-smoke` copied-runtime metrics | Phase 10 zoom 50/100/300 p95 and five-process resource/leak matrix PASS；Phase 10 final cold/warm requalification recorded separately；machine-specific values are evidence, not portable promises |
| `11_testing_and_release.md` | 逐阶段验证、JSON evidence、发布形态 | `phase-00.md`..`phase-10.md`、release 验收清单 | `tools/stickymd-smoke`; `tools/smoke/*.ps1`; `.github/workflows/ci.yml` | Rust task/gate/evidence authority, schema-v1 JSON, structured startup/zoom metrics and CI headless de-duplication tests PASS；manual modes remain excluded/NOT TESTED |
