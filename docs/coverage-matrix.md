...
| `10_performance_reliability.md` | 质量属性 | AC-022、AC-032、空闲/内存观察、P14-A03/A08/A15/A23 | core/source/preview/math/image/export/window/zoom Release baselines；`stickymd-smoke` copied-runtime metrics/attribution；`docs/report/{phase-14-memory-attribution,phase-14-global-module-review,phase-14-global-module-rereview}.md` | v0.1.0 采用 180 preferred / 400 engineering diagnostic / 550 hard 三层 startup policy；正式 warm-cache 进程间隔为 1000 ms，250 ms rapid-restart 仅作非门控诊断；当前 dirty-worktree focused Source/Preview/Split PWS 约 13.0/15.4/21.0 MiB 且 idle CPU p95 ≤0.0053% PASS；关闭 search 后释放约 2 MiB 上限的匹配投影且停止 O(n) 隐式扫描；新 exact candidate 仍需完整资源重跑 |
| `11_testing_and_release.md` | 逐阶段验证、结果驱动等待、可证明并发、GUI child 进程隔离、exact-artifact JSON evidence、发布形态 | `phase-00.md`..`phase-14.md`、P14-A31、release 验收清单 | `tools/stickymd-smoke`; `tools/smoke/*.ps1`; `.github/workflows/ci.yml`; `dist/evidence/*` | Rust task/gate/evidence authority、headless isolated shards 与 targeted module 已落地；所有 smoke GUI child 由 RAII owner 在错误返回/unwind 时 kill + wait，Performance/Resources 对遗留 smoke-owned 测试实例只读预检并 fail closed；同桌面输入/资源通道仍禁止四开并发，新 exact evidence pending |
