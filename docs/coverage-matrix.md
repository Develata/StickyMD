...
| `11_testing_and_release.md` | 逐阶段验证、结果驱动等待、可证明并发、GUI child 进程隔离、Source Freeze、remote artifact promotion、exact-artifact JSON evidence、发布形态 | `phase-00.md`..`phase-14.md`、P12-A09/A11/A13/A14/A18、P14-A31..A34、release 验收清单 | `tools/stickymd-smoke`; `tools/smoke/*.ps1`; `.github/workflows/{ci,release,promote-release}.yml`; `dist/evidence/*`; `dist/exact-candidate/*` | Rust task/gate/evidence authority、headless isolated shards 与 targeted module 已落地；Option B 已批准：本地 build 只作 source preflight，远端 workflow artifact 经校验后成为唯一 Promoted Candidate；最小 exact 重验保留 source-only evidence并重建 artifact-bound evidence；tag/draft/publish 必须复用已验收 artifact 且不得重建；新工具实现与新 exact evidence pending |
