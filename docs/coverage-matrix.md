...
| `09_windows_shell.md` | dock、托盘、置顶、透明度、主题、多显示器 | AC-019..029 | `stickymd-win/{flow/window,config/coordinator.rs,app/{window_runtime,window_interaction,window_geometry_runtime,controls}.rs,platform/windows/{tray,monitor,native_message,window_opacity,window_topmost}.rs}` | Phase 8 pure reducer/geometry/platform tests + copied-Release HWND lifecycle + resource routes；real tray, monitor topology, mixed-DPI, IME and visual rows remain NOT TESTED |
| `10_performance_reliability.md` | 质量属性 | AC-022、空闲/内存观察 | core/source/preview/math/image/export/window Release baselines | Phase 8 adds 100k reducer/geometry/control baselines plus visible/collapsed/hidden process measurements and repeated-shell-cycle checks；machine-specific values are report evidence, not portable promises |
| `11_testing_and_release.md` | 逐阶段验证、发布形态 | `phase-00.md`..`phase-09.md`、release 验收清单 | `stickymd-smoke`; `tools/smoke/*.ps1`; Windows CI | Phase 0–8 稳定入口已实现；Phase 9 convergence/release/package routes are in progress；CI continues to exclude real-desktop/manual modes；人工门见各 Phase 矩阵 |
