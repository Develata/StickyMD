...
| `06_markdown_math_rendering.md` | 预览、数学、raw HTML、remote 图片 | AC-013..017 | `stickymd-render/src/{preview,math}/*`; `stickymd-win/{preview,flow/preview.rs,app/{preview_runtime,preview_input}.rs,platform/windows/shell.rs}` | Phase 5 owned AST/native Preview + Phase 6 RaTeX direct painter/caches/resource matrix automated；formula visual and same-process first-use memory manual NOT TESTED；image decode deferred |
...
| `10_performance_reliability.md` | 质量属性 | AC-022、空闲/内存观察 | core/source/preview/math Release baselines | Phase 6 math Preview warm total p95: 20 KiB 11.189 ms、100 KiB 46.324 ms、1 MiB 712.252 ms；six-state Windows resource automation PASS；same-process first-formula delta NOT TESTED |
| `11_testing_and_release.md` | 逐阶段验证、发布形态 | `phase-00.md`..`phase-06.md`、release 验收清单 | `stickymd-smoke`; `tools/smoke/*.ps1`; Windows CI | Rust 合并任务图 + Phase 0–6 稳定入口已实现；人工门见各 Phase 矩阵 |
