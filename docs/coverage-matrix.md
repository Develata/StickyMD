...
| `11_testing_and_release.md` | 逐阶段验证、exact-artifact JSON evidence、发布形态 | `phase-00.md`..`phase-14.md`、release 验收清单 | `tools/stickymd-smoke`; `tools/smoke/*.ps1`; `.github/workflows/ci.yml`; `dist/evidence/*` | Rust task/gate/evidence authority、independent channel collection、typed hidden-window/view-mode/lifecycle reducer、source-projection gate、至少 100-run 的 desktop-jitter `≥98%` PASS / `<98%` FAIL disposition、Tier A/B/C readiness、G1/G2 guided manual、G3/G4/G5 exact-candidate automation、candidate-bound screenshot hash verification、unsigned receipt contract；tracked tooling 变化后 dynamic evidence pending new exact Phase 14 candidate |
