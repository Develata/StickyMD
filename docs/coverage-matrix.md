...
| `10_performance_reliability.md` | 质量属性 | AC-022、AC-032、空闲/内存观察 | core/source/preview/math/image/export/window/zoom Release baselines；`stickymd-smoke` copied-runtime metrics | Phase 11 cold 300.692 ms / warm 311.353 ms both satisfy the USER-approved v0.1.0 400 ms hard boundaries；warm 180 ms preferred target remains unmet；other performance/resource hard gates PASS |
| `11_testing_and_release.md` | 逐阶段验证、exact-artifact JSON evidence、发布形态 | `phase-00.md`..`phase-12.md`、release 验收清单 | `tools/stickymd-smoke`; `tools/smoke/*.ps1`; `.github/workflows/ci.yml`; `dist/evidence/*` | Rust task/gate/evidence authority, Phase 12 candidate/manual/remote/downloaded/readiness identity binding and CI headless de-duplication；manual rows remain NOT TESTED until exact receipt |
